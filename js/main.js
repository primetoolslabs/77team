...
decideInitialScreen();
on("openFirstDevSetup","click",()=>showOnly("setupScreen"));
on("cancelFirstDevSetup","click",()=>showOnly("authScreen"));
...
async function createPresenceBackup({automatic=false}={}){
  ...
  const payload={week,counts,total:records.length,rtTotal:rtRecords.length,automatic,backupSchema:2,status:"writing",createdBy:state.user.uid,createdByName:state.profile?.name||state.user.email,createdAt:serverTimestamp(),createdAtText:now.toISOString(),sourceVersion:"22.9.19"};
...
async function downloadPresenceBackup(id){const item=state.presenceBackups.find(row=>row.id===id);if(!item)return toast("Backup não encontrado.");const data=await loadPresenceBackupData(item);downloadJson(`presenca-${item.week||id}.json`,{version:"22.9.19",exportedAt:new Date().toISOString(),backup:{...item,...data}});}
...
if("serviceWorker" in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=22.9.19").catch(error=>console.warn("Service Worker indisponível:",error)));
...
on("profileNicknameForm","submit",async event=>{
  ...
  try{
    ...
    toast(error?.code==="permission-denied"
      ? "Permissão negada ao salvar o perfil. Publique o firestore.rules da V22.9.19 no Firebase e confirme o projeto team-f78cd."
...
function validateBackupPayload(payload){
  ...
  if(Number(payload.backupSchema||0)!==3)throw new Error("Schema de backup incompatível. Use um backup completo do Firestore V22.8.5 a V22.9.19 com schema 3.");
  if(!["22.8.5","22.8.6","22.8.7","22.8.8","22.8.9","22.9.0","22.9.1","22.9.2","22.9.3","22.9.4","22.9.5","22.9.6","22.9.7","22.9.8","22.9.9","22.9.10","22.9.11","22.9.12","22.9.13","22.9.14","22.9.15","22.9.16","22.9.17","22.9.18","22.9.19"].includes(String(payload.version||"")))throw new Error("Versão de backup incompatível.");
  ...
  for(const claim of payload.collections.nicknameClaims){
    ...
    if(["22.9.12","22.9.13","22.9.14","22.9.15","22.9.16","22.9.17","22.9.18","22.9.19"].includes(String(payload.version||""))&&String(user.memberDocumentId||"")!==String(claim.memberId||""))throw new Error(`Documento de membro divergente na reserva ${claim.id}.`);
    if(["22.9.12","22.9.13","22.9.14","22.9.15","22.9.16","22.9.17","22.9.18","22.9.19"].includes(String(payload.version||""))&&normalizeAccessRole(user.accessRole||user.role)!=="dev"&&!claim.memberId)throw new Error(`Reserva sem membro vinculado: ${claim.id}.`);
  ...
  if(["22.9.11","22.9.12","22.9.13","22.9.14","22.9.15","22.9.16","22.9.17","22.9.18","22.9.19"].includes(String(payload.version||"")))for(const user of payload.collections.users){
  ...
  for(const [backupId,data] of Object.entries(groups)){
    ...
    if(["22.9.17","22.9.18","22.9.19"].includes(String(payload.version))&&parent.status!=="completed")throw new Error(`Backup semanal incompleto: ${backupId}.`);
