...
const state={user:null,profile:null,guest:false,members:[],attendance:[],users:[],audit:[],events:[],notifications:[],sentNotifications:[],notificationReads:[],settings:{},xpLogs:[],unsubs:[]};
...
function populateNotificationUsers(){
  ...
  select.innerHTML='<option value="">Selecione um usuário</option>'+users.map(u=>`<option value="${u.id}">${escapeHtml(u.name||u.email)} · ${u.role==="owner"?"Proprietário":u.role==="staff"?"Staff":"Membro"}</option>`).join("");
