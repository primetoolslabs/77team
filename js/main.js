...
function renderLevelSystem(){
  ...
  if(state.user&&!state.guest){
    ...
    setText("profileLevel",`Level ${progress.level}`);
    setText("profileLevelMetric",progress.level);
    setText("profileLevelTitle",progress.title);
    setText("profileXpTotal",`${progress.totalXp.toLocaleString("pt-BR")} XP total`);
    setText("profileXpCurrent",`${progress.currentXp.toLocaleString("pt-BR")} / ${progress.requiredXp.toLocaleString("pt-BR")} XP`);
    setText("profileNextLevel",`${Math.max(0,progress.requiredXp-progress.currentXp).toLocaleString("pt-BR")} XP para o próximo Level`);
