...
on("checkUpdatesButton","click",()=>{setText("updateStatusText","V30.1 Dashboard Revolution instalada. Compatibilidade preservada com a base V21.2.4.");toast("Verificação local concluída.")});
on("createBackupButton","click",()=>{if(!owner())return;downloadJson(`77-team-backup-${new Date().toISOString().slice(0,10)}.json`,{version:"30.1-dashboard",baseVersion:"21.2.4",exportedAt:new Date().toISOString(),members:state.members,attendance:state.attendance,users:state.users,events:state.events,notifications:state.sentNotifications,audit:state.audit,settings:state.settings});toast("Backup JSON gerado.")});
