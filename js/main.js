...
const state={user:null,profile:null,guest:false,members:[],attendance:[],users:[],audit:[],events:[],notifications:[],notificationReads:[],settings:{},xpLogs:[],unsubs:[]};
...
const STAFF_ONLY_PAGES=new Set(["worldboss","purgatorio","eventos","personagens","metas","solicitacoes","notificacoes"]);
const OWNER_ONLY_PAGES=new Set(["staff","configuracoes","auditoria"]);
...
function openAllowedPage(page){
  if(!canOpenPage(page)){
    toast(OWNER_ONLY_PAGES.has(page)?"Esta área é exclusiva do Proprietário.":"Esta área é exclusiva do Proprietário e da Staff.");
...
function subscribeAll(){
  ...
  if(state.user){
    state.unsubs.push(onSnapshot(collection(db,"notificationReads"),s=>{
      state.notificationReads=s.docs.map(d=>({id:d.id,...d.data()})).filter(r=>r.userId===state.user.uid);
      render();
    }));
    state.unsubs.push(onSnapshot(collection(db,"notifications"),s=>{
      state.notifications=s.docs.map(d=>({id:d.id,...d.data()})).filter(notificationVisibleToCurrentUser);
      render();
...
function notificationVisibleToCurrentUser(n){
  ...
  if(target==="staff")return owner()||staff();
...
function renderNotifications(){
  ...
  setHtml("notificationRows",rows.map(n=>{
    ...
    return `<article class="notification-item ${read?"":"unread"}" data-notification-id="${n.id}">
      <div><strong>${n.title||"Notificação"}</strong><p>${n.message||""}</p><small>${n.createdByName||"77 TEAM"} · ${notificationDate(n)}</small></div>
...
function render(){
  ...
  $("#memberRows").innerHTML=state.members.map(member=>{
    ...
    return `<tr>
      ...
      <td>
        ${owner()&&linkedUserForMember(member)?`
          <button class="btn" data-toggle-staff="${member.id}" type="button">
            ${linkedUserForMember(member)?.role==="staff"?"Remover Staff":"Tornar Staff"}
          </button>
        `:""}
...
document.addEventListener("click",async e=>{
  ...
  if(toggleStaff){
    if(!owner())return toast("Somente o Proprietário pode alterar o acesso Staff.");

    const member=state.members.find(item=>item.id===toggleStaff.dataset.toggleStaff);
    const user=linkedUserForMember(member);

    if(!member||!user){
      return toast("Este membro não possui uma conta vinculada.");
    }

    if(user.role==="owner"){
      return toast("O cargo do Proprietário não pode ser alterado.");
    }

    const promote=user.role!=="staff";
    const nextUserRole=promote?"staff":"member";
    const nextMemberRole=promote?"Staff":memberRoleFromAccessRole("member",member.role);
    ...
    try{
      ...
      await batch.commit();
      await audit(
        promote?"membro promovido a Staff":"Staff rebaixado para membro",
        `${member.name} · ${user.email||""}`
      );

      toast(promote
        ?`${member.name} agora possui acesso Staff.`
        :`${member.name} voltou ao acesso de membro.`
      );
    }catch(error){
      toast(errMsg(error));
    }
  ...
  if(approve&&editor()){
    ...
    const accessRole=accessRoleFromMemberRole(role);

    if(accessRole==="staff"&&!owner()){
      return toast("Somente o Proprietário pode promover um usuário para Staff.");
    }
    ...
    await batch.commit();
    await audit(
      accessRole==="staff"?"usuário promovido a Staff":"membro aprovado",
      u.email
    );
  }
});
...
async function saveSettingsSection(section){
  if(!owner())return toast("Apenas o Proprietário pode alterar configurações.");
  ...
  grid.innerHTML=staffRows().map(item=>`
    <article class="staff-card">
      ...
      <div class="staff-card-copy">
        <h3>${item.name}</h3>
        <p>${item.role==="owner"?"Proprietário":"Staff"} · ${item.clan}</p>
...
function printStaffReport(){
  ...
  const body=rows.map(item=>`<tr>
    <td>${item.name}</td><td>${item.role==="owner"?"Proprietário":"Staff"}</td>
...
function downloadStaffCsvFile(){
  ...
  const lines=[
    ...
    ...rows.map(item=>[
      item.name,
      item.role==="owner"?"Proprietário":"Staff",
      item.clan,
      item.email,
      item.stat.present,
      item.stat.absent,
...
function populateNotificationUsers(){
  ...
  select.innerHTML='<option value="">Selecione um usuário</option>'+users.map(u=>`<option value="${u.id}">${u.name||u.email} · ${u.role==="owner"?"Proprietário":u.role==="staff"?"Staff":"Membro"}</option>`).join("");
...
function renderNotificationAdmin(){
  ...
  const sent=state.notifications.filter(n=>n.createdBy===state.user?.uid||owner()).slice().sort((a,b)=>{const av=a.createdAt?.toMillis?.()||Date.parse(a.createdAt||0)||0;const bv=b.createdAt?.toMillis?.()||Date.parse(b.createdAt||0)||0;return bv-av});
  setText("notificationSentCount",sent.length);
  list.innerHTML=sent.map(n=>`<article class="notification-sent-item type-${n.type||"info"}">
    <div><strong>${n.title||"Notificação"}</strong><p>${n.message||""}</p><small>${n.targetType==="user"?`Individual: ${n.targetUserName||"Usuário"}`:"Todos os usuários"} · ${notificationDate(n)}</small></div>
...
on("notificationAdminForm","submit",async event=>{
  ...
  try{
    await addDoc(collection(db,"notifications"),{
      title:String(byId("notificationTitle")?.value||"").trim(),
      message:String(byId("notificationMessage")?.value||"").trim(),
