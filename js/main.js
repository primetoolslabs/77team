...
async function createPresenceBackup({automatic=false}={}){
  ...
  const payload={week,counts,total:records.length,rtTotal:rtRecords.length,automatic,backupSchema:2,status:"writing",createdBy:state.user.uid,createdByName:state.profile?.name||state.user.email,createdAt:serverTimestamp(),createdAtText:now.toISOString(),sourceVersion:"22.9.29"};
...
async function downloadPresenceBackup(id){const item=state.presenceBackups.find(row=>row.id===id);if(!item)return toast("Backup não encontrado.");const data=await loadPresenceBackupData(item);downloadJson(`presenca-${item.week||id}.json`,{version:"22.9.29",exportedAt:new Date().toISOString(),backup:{...item,...data}});}
  ...
  $("#memberRows").innerHTML=visibleMembers().map(member=>{
    ...
    return `<tr>
      ...
      <td>
        ...
        ${permissionEnabled("members_clan_change")?`<div class="member-role-control">
...
Um registro será salvo em Gestão → RT Presença.`))return;
  ...
  if(changeClan){
    if(!permissionEnabled("members_clan_change"))return toast("Sem permissão para alterar o clã.");
...
if("serviceWorker" in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=22.9.29").catch(error=>console.warn("Service Worker indisponível:",error)));
...
on("profileNicknameForm","submit",async event=>{
  ...
  try{
    ...
    toast(error?.code==="permission-denied"
      ? "Permissão negada ao salvar o perfil. Publique o firestore.rules da V22.9.29 no Firebase e confirme o projeto team-f78cd."
...
function validateBackupPayload(payload){
  ...
  if(Number(payload.backupSchema||0)!==3)throw new Error("Schema de backup incompatível. Use um backup completo do Firestore V22.8.5 a V22.9.29 com schema 3.");
  if(!["22.8.5","22.8.6","22.8.7","22.8.8","22.8.9","22.9.0","22.9.1","22.9.2","22.9.3","22.9.4","22.9.5","22.9.6","22.9.7","22.9.8","22.9.9","22.9.10","22.9.11","22.9.12","22.9.13","22.9.14","22.9.15","22.9.16","22.9.17","22.9.18","22.9.19","22.9.20","22.9.21","22.9.22","22.9.23","22.9.24","22.9.25","22.9.26","22.9.27","22.9.28","22.9.29"].includes(String(payload.version||"")))throw new Error("Versão de backup incompatível.");
  ...
  if(!Array.isArray(payload.collections.payments)&&!["22.9.23","22.9.24","22.9.25","22.9.26","22.9.27","22.9.28","22.9.29"].includes(String(payload.version||"")))payload.collections.payments=[];
  ...
  if(["22.9.23","22.9.24","22.9.25","22.9.26","22.9.27","22.9.28","22.9.29"].includes(String(payload.version||""))&&payload.collections.payments.some(item=>!Number.isSafeInteger(item.quantity)||item.quantity<1||item.quantity>99000000))throw new Error("Pagamento sem quantidade válida no backup.");
  ...
  for(const claim of payload.collections.nicknameClaims){
    ...
    if(["22.9.12","22.9.13","22.9.14","22.9.15","22.9.16","22.9.17","22.9.18","22.9.19","22.9.20","22.9.21","22.9.22","22.9.23","22.9.24","22.9.25","22.9.26","22.9.27","22.9.28","22.9.29"].includes(String(payload.version||""))&&String(user.memberDocumentId||"")!==String(claim.memberId||""))throw new Error(`Documento de membro divergente na reserva ${claim.id}.`);
    if(["22.9.12","22.9.13","22.9.14","22.9.15","22.9.16","22.9.17","22.9.18","22.9.19","22.9.20","22.9.21","22.9.22","22.9.23","22.9.24","22.9.25","22.9.26","22.9.27","22.9.28","22.9.29"].includes(String(payload.version||""))&&normalizeAccessRole(user.accessRole||user.role)!=="dev"&&!claim.memberId)throw new Error(`Reserva sem membro vinculado: ${claim.id}.`);
  ...
  if(["22.9.11","22.9.12","22.9.13","22.9.14","22.9.15","22.9.16","22.9.17","22.9.18","22.9.19","22.9.20","22.9.21","22.9.22","22.9.23","22.9.24","22.9.25","22.9.26","22.9.27","22.9.28","22.9.29"].includes(String(payload.version||"")))for(const user of payload.collections.users){
  ...
  for(const [backupId,data] of Object.entries(groups)){
    ...
    if(["22.9.17","22.9.18","22.9.19","22.9.20","22.9.21","22.9.22","22.9.23","22.9.24","22.9.25","22.9.26","22.9.27","22.9.28","22.9.29"].includes(String(payload.version))&&parent.status!=="completed")throw new Error(`Backup semanal incompleto: ${backupId}.`);
