...
const state={user:null,profile:null,guest:false,members:[],attendance:[],users:[],audit:[],events:[],notifications:[],sentNotifications:[],notificationReads:[],settings:{},xpLogs:[],supportMessages:[],selectedSupportOwnerUid:"",selectedSupportTicketId:"",supportView:"active",chatMessages:[],selectedChatOwnerUid:"",chatSearch:"",unsubs:[]};
...
// V20.2 - Chat privado individual entre responsáveis e usuários
function chatTime(item){
  const d=item?.createdAt?.toDate?.();
  return d?d.toLocaleString("pt-BR",{dateStyle:"short",timeStyle:"short"}):"Agora";
}
function chatMessagesFor(uid){return state.chatMessages.filter(m=>m.ownerUid===uid).sort((a,b)=>(a.createdAt?.toMillis?.()||0)-(b.createdAt?.toMillis?.()||0))}
function chatOwners(){
  const map=new Map();
  state.chatMessages.forEach(m=>map.set(m.ownerUid,{uid:m.ownerUid,name:m.ownerName||"Usuário",last:m}));
  return [...map.values()].sort((a,b)=>(b.last.createdAt?.toMillis?.()||0)-(a.last.createdAt?.toMillis?.()||0));
}
...
function renderPrivateChat(){
  if(!state.user)return;
  const ownMsgs=chatMessagesFor(state.user.uid);
  setHtml("profileChatMessages",ownMsgs.map(renderChatBubble).join("")||'<p class="empty-state">Nenhuma conversa privada iniciada.</p>');
  setText("profileChatBadge",ownMsgs.length?`${ownMsgs.length} mensagens`:"Sem mensagens");
  ...
  const owners=chatOwners();
  const ownerIds=new Set(owners.map(x=>x.uid));
  const rows=[...owners,...users.filter(u=>!ownerIds.has(u.id)).map(u=>({uid:u.id,name:u.name||u.email,last:null}))];
  setHtml("chatUserList",rows.map(x=>`<button type="button" class="support-conversation ${state.selectedChatOwnerUid===x.uid?"active":""}" data-chat-owner="${escapeHtml(x.uid)}"><strong>${escapeHtml(x.name)}</strong><span>${escapeHtml(x.last?.text||"Iniciar nova conversa")}</span><small>${x.last?chatTime(x.last):"Sem mensagens"}</small></button>`).join("")||'<p class="empty-state">Nenhum usuário encontrado.</p>');
  const selected=state.selectedChatOwnerUid;
  const selectedUser=state.users.find(u=>u.id===selected);
  setText("chatSelectedTitle",selected?(selectedUser?.name||selectedUser?.email||"Usuário"):"Selecione um usuário");
  const msgs=selected?chatMessagesFor(selected):[];
  ...
  if(byId("chatAdminForm"))byId("chatAdminForm").classList.toggle("hidden",!selected);
  const box=byId("chatAdminMessages");if(box)requestAnimationFrame(()=>box.scrollTop=box.scrollHeight);
}
async function uploadChatImage(file,ownerUid){
  if(!file)return {url:"",path:""};
  if(!["image/png","image/jpeg","image/webp"].includes(file.type))throw new Error("Use uma imagem PNG, JPG ou WEBP.");
  if(file.size>5*1024*1024)throw new Error("A imagem deve ter no máximo 5 MB.");
  const safe=file.name.replace(/[^a-zA-Z0-9._-]/g,"_");
  const path=`chat/${ownerUid}/${Date.now()}-${safe}`;
  const ref=storageRef(storage,path);
  const snap=await uploadBytes(ref,file,{contentType:file.type});
  return {url:await getDownloadURL(snap.ref),path};
}
async function sendPrivateChat({ownerUid,ownerName,text,link,file}){
  const cleanText=(text||"").trim();
  if(!cleanText&&!file)throw new Error("Digite uma mensagem ou selecione uma imagem.");
  const normalizedLink=(link||"").trim();
  if(normalizedLink&&!/^https:\/\//i.test(normalizedLink))throw new Error("O link precisa começar com https://");
  const image=await uploadChatImage(file,ownerUid);
  await addDoc(collection(db,"chatMessages"),{ownerUid,ownerName,senderUid:state.user.uid,senderName:state.profile?.name||state.user.email,senderRole:editor()?normalizeAccessRole(state.profile?.role):"member",text:cleanText,link:normalizedLink,imageUrl:image.url,imagePath:image.path,createdAt:serverTimestamp()});
}
...
on("chatAdminForm","submit",async e=>{e.preventDefault();if(!editor()||!state.selectedChatOwnerUid)return;const user=state.users.find(u=>u.id===state.selectedChatOwnerUid);try{await sendPrivateChat({ownerUid:state.selectedChatOwnerUid,ownerName:user?.name||user?.email||"Usuário",text:byId("chatAdminText").value,link:byId("chatAdminLink").value,file:byId("chatAdminImage").files?.[0]});e.target.reset();byId("chatAdminText")?.focus();await audit("Chat privado",`Mensagem enviada para ${user?.name||user?.email||state.selectedChatOwnerUid}`);toast("Mensagem enviada.");}catch(error){toast(error.message||errMsg(error))}});
on("chatSearch","input",renderPrivateChat);
document.addEventListener("click",e=>{const b=e.target.closest("[data-chat-owner]");if(!b)return;state.selectedChatOwnerUid=b.dataset.chatOwner;renderPrivateChat()});
