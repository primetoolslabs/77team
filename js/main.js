...
function applyPermissions(){
  ...
  $("#userBadge").textContent=state.guest?"Visitante":`${state.profile?.name||state.profile?.email} · ${state.profile?.role}`;
}
...
function render(){
  ...
  const present=state.attendance.filter(x=>x.status===1).length,marked=state.attendance.filter(x=>x.status!==0).length;
  $("#kPresence").textContent=present;$("#kRate").textContent=(marked?Math.round(present/marked*100):0)+"%";
  const rank=state.members.map(m=>({...m,...stats(m.name)})).sort((a,b)=>b.rate-a.rate||b.present-a.present);
