...
function subscribeAll(){
  subscribePublic();
  if(editor())state.unsubs.push(onSnapshot(collection(db,"users"),s=>{state.users=s.docs.map(d=>({id:d.id,...d.data()}));render();scheduleAttendanceUserMigration()}));
...
function render(){
  ...
  $("#kBestPoints").textContent=`${rank[0]?.present||0} pontos`;
  ...
  $("#recentPresenceRows").innerHTML=recent.map(a=>`<tr>
    ...
    <td>${formatHistoryDate(a.date||"")}</td>
    <td>${a.slot||a.kind||"—"}</td>
    <td><span class="dashboard-status present">✓ Presente</span></td>
  ...
  $("#dashboardMemberRows").innerHTML=dashboardMembers.map(member=>{
    const xp=progressionFor(member);
    return `<tr>
      <td><span class="member-avatar">${(member.name||"?").slice(0,1).toUpperCase()}</span><strong>${member.name}</strong></td>
      <td>${roleBadge(member.role)}</td>
      <td>${member.clan||"—"}</td>
      <td><strong>Lv. ${xp.level}</strong></td>
      <td>${xp.totalXp.toLocaleString("pt-BR")}</td>
      <td>${member.present}</td>
      <td><strong class="ranking-points">${member.rate}%</strong></td>
      <td><span class="online-status"><i></i>${member.active===false?"Inativo":"Ativo"}</span></td>
      <td><button class="dashboard-row-action" data-view-member="${member.id}" type="button">•••</button></td>
    </tr>`;
  ...
  $("#memberRows").innerHTML=state.members.map(member=>{
    ...
    return `<tr>
      ...
      <td>${editor()?`<button class="btn danger" data-delete-member="${member.id}">Excluir</button>`:"Visualização"}</td>
    </tr>`;
