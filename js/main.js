...
const {getFirestore,collection,doc,getDoc,getDocs,setDoc,addDoc,updateDoc,deleteDoc,onSnapshot,serverTimestamp,writeBatch}=await import(`${SDK}/firebase-firestore.js`);
...
function render(){
  ...
  renderNotifications();renderCalendar();renderStatistics();updatePdfMemberOptions();renderOwnProfile();renderCharacterProfile();renderCharactersTable();renderCharacterCenter();renderHistoryCenter();renderGoals();renderSystemHealth();applyRestrictedVisibility();
}
...
on("profileNicknameForm","submit",async event=>{
  ...
  try{
    ...
    state.profile.name=name;applyPermissions();renderOwnProfile();renderCharacterProfile();renderCharactersTable();renderCharacterCenter();renderHistoryCenter();renderGoals();renderSystemHealth();applyRestrictedVisibility();toast("Nickname atualizado.");
...
on("profileAvatarForm","submit",async event=>{
  ...
  try{
    ...
    renderOwnProfile();renderCharacterProfile();renderCharactersTable();renderCharacterCenter();renderHistoryCenter();renderGoals();renderSystemHealth();applyRestrictedVisibility();
    applyPermissions();
    event.target.reset();
...
on("removeProfileAvatar","click",async()=>{
  try{
    ...
    state.profile.avatarDataUrl="";renderOwnProfile();renderCharacterProfile();renderCharactersTable();renderCharacterCenter();renderHistoryCenter();renderGoals();renderSystemHealth();applyRestrictedVisibility();toast("Avatar removido.");
