...
const state={user:null,profile:null,guest:false,onboardingRequired:false,members:[],attendance:[],users:[],audit:[],events:[],notifications:[],sentNotifications:[],notificationReads:[],settings:{},xpLogs:[],supportMessages:[],selectedSupportOwnerUid:"",selectedSupportTicketId:"",supportView:"active",chatMessages:[],selectedChatOwnerUid:"",selectedChatId:"",chatView:"active",chatSearch:"",editingCharacterUserId:"",presenceFilters:{},unsubs:[]};
...
const STAFF_ONLY_PAGES=new Set(["worldboss","purgatorio","eventos","personagens","metas","solicitacoes","notificacoes","atendimento","chat"]);
...
function presenceStatus(status){return ({"1":{label:"Presente",icon:"✓",cls:"present"},"2":{label:"Atrasado",icon:"◷",cls:"late"},"3":{label:"Justificado",icon:"!",cls:"justified"},"-1":{label:"Ausente",icon:"×",cls:"absent"},"0":{label:"Pendente",icon:"—",cls:"pending"}})[String(status||0)]}
...
function renderPresence(targetId,kind){
  const target=$("#"+targetId),filter=presenceFilter(kind),allSlots=TYPES[kind],slots=filter.slot==="all"?allSlots:allSlots.filter(x=>x===filter.slot);
  ...
  const rows=[];members.forEach(m=>slots.forEach(slot=>rows.push(presenceRecord(kind,m.id,slot,filter.date))));
  const counts={present:rows.filter(r=>r?.status===1).length,late:rows.filter(r=>r?.status===2).length,justified:rows.filter(r=>r?.status===3).length,absent:rows.filter(r=>r?.status===-1).length,pending:rows.filter(r=>!r||!r.status).length};
  const considered=counts.present+counts.late+counts.absent,rate=considered?Math.round((counts.present+counts.late)/considered*100):0;
  const title=kind==="worldboss"?"WorldBoss":kind==="purgatorio"?"Purgatório":"Eventos";
  target.innerHTML=`<div class="presence-command" data-presence-root="${kind}">
    <div class="presence-filterbar">
      <label>Data<input type="date" value="${filter.date}" data-presence-filter="date" data-kind="${kind}"></label>
      <label>Semana<input type="week" value="${filter.week}" data-presence-filter="week" data-kind="${kind}"></label>
      <label>Horário/Evento<select data-presence-filter="slot" data-kind="${kind}"><option value="all">Todos</option>${allSlots.map(x=>`<option value="${escapeHtml(x)}" ${filter.slot===x?"selected":""}>${escapeHtml(x)}${kind==="eventos"?` · ${eventDay(x)}`:""}</option>`).join("")}</select></label>
      <label>Clã<select data-presence-filter="clan" data-kind="${kind}"><option value="">Todos os clãs</option>${CLANS.map(x=>`<option ${filter.clan===x?"selected":""}>${escapeHtml(x)}</option>`).join("")}</select></label>
      <label class="presence-search">Buscar<input type="search" value="${escapeHtml(filter.search)}" placeholder="Nome do membro" data-presence-filter="search" data-kind="${kind}"></label>
    </div>
    <div class="presence-summary">
      <article><span>Registros</span><strong>${rows.length}</strong></article><article class="ok"><span>Presentes</span><strong>${counts.present}</strong></article><article class="warn"><span>Atrasados</span><strong>${counts.late}</strong></article><article class="info"><span>Justificados</span><strong>${counts.justified}</strong></article><article class="danger"><span>Ausentes</span><strong>${counts.absent}</strong></article><article><span>Pendentes</span><strong>${counts.pending}</strong></article><article><span>Participação</span><strong>${rate}%</strong></article>
    </div>
    ${editor()?`<div class="presence-actions"><button class="btn primary" data-presence-bulk="1" data-kind="${kind}">✓ Marcar presentes</button><button class="btn" data-presence-bulk="-1" data-kind="${kind}">× Marcar ausentes</button><button class="btn" data-presence-bulk="0" data-kind="${kind}">Limpar marcações</button><button class="btn" data-presence-review="${kind}">Revisar e salvar</button></div>`:""}
    <div class="presence-legend"><span>✓ Presente</span><span>◷ Atrasado</span><span>! Justificado</span><span>× Ausente</span><span>— Pendente</span><small>Alterações são salvas automaticamente.</small></div>
    <div class="presence-grid"><table><thead><tr><th>Jogador</th><th>Clã</th>${slots.map(x=>`<th><span>${escapeHtml(x)}</span>${kind==="eventos"?`<small>${eventDay(x)}</small>`:""}</th>`).join("")}</tr></thead><tbody>${members.map(m=>`<tr><td><button class="presence-member-link" data-presence-history="${m.id}">${escapeHtml(m.name)}</button></td><td>${escapeHtml(m.clan||"—")}</td>${slots.map(slot=>{const row=presenceRecord(kind,m.id,slot,filter.date),st=presenceStatus(row?.status||0);return `<td><div class="presence-cell"><button ${editor()?"":"disabled"} class="presence-btn ${st.cls}" title="${st.label}${row?.updatedByName?` · Editado por ${escapeHtml(row.updatedByName)}`:""}" data-presence="${kind}|${m.id}|${slot}|${filter.date}">${st.icon}<small>${st.label}</small></button>${editor()?`<button class="presence-note-btn ${row?.note?"has-note":""}" title="${escapeHtml(row?.note||"Adicionar observação")}" data-presence-note="${kind}|${m.id}|${slot}|${filter.date}">📝</button>`:""}${row?.updatedAt?`<small class="presence-edited">${row.updatedByName?escapeHtml(row.updatedByName):"Equipe"}</small>`:""}</div></td>`}).join("")}</tr>`).join("")||`<tr><td colspan="${slots.length+2}">Nenhum membro encontrado com os filtros atuais.</td></tr>`}</tbody></table></div>
  </div>`;
}
...
document.addEventListener("click",async e=>{
  ...
  if(review&&editor()){
    const kind=review.dataset.presenceReview,filter=presenceFilter(kind),slots=filter.slot==="all"?TYPES[kind]:[filter.slot],members=state.members.filter(m=>(!filter.clan||m.clan===filter.clan)&&(!filter.search||String(m.name||"").toLowerCase().includes(filter.search.toLowerCase()))),pending=members.reduce((n,m)=>n+slots.filter(slot=>!presenceRecord(kind,m.id,slot,filter.date)?.status).length,0);
    if(pending&&!confirm(`${pending} registros ainda estão pendentes. Deseja finalizar a revisão mesmo assim?`))return;await audit("presença revisada",`${kind} · ${filter.date} · ${pending} pendentes`);toast(pending?"Revisão registrada com pendências.":"Presença revisada e completa.");
  }
...
on("profileNicknameForm","submit",async event=>{
  ...
  try{
    ...
    state.profile.name=name;applyPermissions();updateFirstAccessUI();renderOwnProfile();renderCharacterProfile();renderCharactersTable();renderCharacterCenter();renderHistoryCenter();renderGoals();renderSystemHealth();renderStaffCommandCenter();applyRestrictedVisibility();toast("Nickname atualizado.");
