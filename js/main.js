...
function openAllowedPage(page){
  if(!canOpenPage(page)){
    toast(OWNER_ONLY_PAGES.has(page)?"Esta área é exclusiva da Liderança.":"Esta área é exclusiva da Liderança e da Staff.");
...
$("#setupForm").onsubmit=async e=>{
  e.preventDefault();
  try{
    if(await ownerExists())throw new Error("O liderança já foi configurado.");
...
document.addEventListener("click",async e=>{
  ...
  if(toggleStaff){
    if(!owner())return toast("Somente a Liderança pode alterar o acesso Staff.");
    ...
    if(user.role==="owner"){
      return toast("O cargo da Liderança não pode ser alterado.");
    }
  ...
  if(approve&&editor()){
    ...
    if(accessRole==="staff"&&!owner()){
      return toast("Somente a Liderança pode promover um usuário para Staff.");
    }
...
document.addEventListener("click",event=>{
  ...
  if(requestedPage&&!canOpenPage(requestedPage)){
    ...
    toast(OWNER_ONLY_PAGES.has(requestedPage)?"Esta área é exclusiva da Liderança.":"Esta área é exclusiva da Liderança e da Staff.");
    window.TeamManagerUI?.activatePage("dashboard");
  }
},true);
