...
const ROLE_PERMISSION_DEFINITIONS=Object.freeze([
  ...
  {group:"Membros",key:"members_delete",label:"Excluir membros",defaults:{dev:true,leadership:true,staff:false,member:false}},
...
function applyPermissions(){
  ...
  ["personagens","solicitacoes","notificacoes","atendimento","chat","metas"].forEach(page=>{
...
function renderPayments(){
  ...
  rowsHost.innerHTML=rows.map(item=>`<tr><td>${escapeHtml(paymentDate(item.createdAt))}</td><td><strong>${escapeHtml(item.nickname||"—")}</strong></td><td>${escapeHtml(item.paymentType||"—")}</td><td>${Number.isSafeInteger(Number(item.quantity))?Number(item.quantity).toLocaleString("pt-BR"):"—"}</td><td>${escapeHtml(item.responsibleNick||"—")}</td><td>${owner()?`<button class="btn danger mini" data-delete-payment="${escapeHtml(item.id)}" type="button">Excluir</button>`:"—"}</td></tr>`).join("")||'<tr><td colspan="6">Nenhum pagamento registrado.</td></tr>';
}
...
function downloadPaymentHistory(rows,label,periodLabel){
  ...
  const body=ordered.map(item=>`<tr><td>${escapePrintHtml(paymentDate(item.createdAt))}</td><td>${escapePrintHtml(item.nickname||"—")}</td><td>${escapePrintHtml(item.paymentType||"—")}</td><td>${escapePrintHtml(Number(item.quantity).toLocaleString("pt-BR"))}</td><td>${escapePrintHtml(item.responsibleNick||"—")}</td></tr>`).join("");
  popup.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>77-team-pagamentos-${escapePrintHtml(label)}.pdf</title><style>@page{size:A4 landscape;margin:10mm}*{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;color:#17131c;margin:0}header{padding:16px;color:#fff;background:#251133;border-bottom:4px solid #a83cff}header strong{color:#d277ff;font-size:21px}h1{font-size:17px;margin:6px 0}.actions{text-align:right;margin:12px 0}.actions button{padding:9px 14px;background:#8e24cf;color:#fff;border:0;border-radius:6px;cursor:pointer}table{width:100%;border-collapse:collapse;font-size:10px}th{background:#2c123c;color:#fff;padding:8px;border:1px solid #7e4597}td{padding:7px;border:1px solid #d5c7dc}tbody tr:nth-child(even){background:#f8f2fb}footer{margin-top:10px;font-size:10px;color:#555}@media print{.actions{display:none}}</style></head><body><header><strong>77 TEAM MANAGER</strong><h1>Histórico de pagamentos — ${escapePrintHtml(periodLabel)}</h1></header><div class="actions"><button data-popup-print>Salvar como PDF</button></div><p>Período: ${escapePrintHtml(periodLabel)} · ${ordered.length} pagamento(s)</p><table><thead><tr><th>Data/Hora</th><th>Nickname</th><th>Pagamento</th><th>Quantidade (1 a 99.000.000)</th><th>Responsável</th></tr></thead><tbody>${body}</tbody></table><footer>Gerado em ${escapePrintHtml(new Date().toLocaleString("pt-BR"))} por 77 TEAM Manager.</footer></body></html>`);
...
async function createPresenceBackup({automatic=false}={}){
  ...
  const payload={week,counts,total:records.length,rtTotal:rtRecords.length,automatic,backupSchema:2,status:"writing",createdBy:state.user.uid,createdByName:state.profile?.name||state.user.email,createdAt:serverTimestamp(),createdAtText:now.toISOString(),sourceVersion:"22.9.25"};
...
async function downloadPresenceBackup(id){const item=state.presenceBackups.find(row=>row.id===id);if(!item)return toast("Backup não encontrado.");const data=await loadPresenceBackupData(item);downloadJson(`presenca-${item.week||id}.json`,{version:"22.9.25",exportedAt:new Date().toISOString(),backup:{...item,...data}});}
...
Um registro será salvo em Gestão → RT Presença.`))return;
  ...
  if(changeClan){
    ...
    try{
      const batch=writeBatch(db),user=linkedUserByUidForMember(member),payload={clan:nextClan,clanUpdatedAt:serverTimestamp(),clanUpdatedBy:state.user.uid,updatedAt:serverTimestamp()};
      ...
      await audit("clã de membro alterado",`${member.name} → ${nextClan||"Sem clã"}${user?" · conta sincronizada por UID":" · membro sem conta vinculada"}`);
      toast(user?"Clã atualizado no membro e na conta.":"Clã atualizado no membro. Vincule o UID para sincronizar a conta.");
    }catch(error){toast(errMsg(error))}
    return;
  }
...
if("serviceWorker" in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=22.9.25").catch(error=>console.warn("Service Worker indisponível:",error)));
...
on("profileNicknameForm","submit",async event=>{
  ...
  try{
    ...
    toast(error?.code==="permission-denied"
      ? "Permissão negada ao salvar o perfil. Publique o firestore.rules da V22.9.25 no Firebase e confirme o projeto team-f78cd."
...
function validateBackupPayload(payload){
  ...
  if(Number(payload.backupSchema||0)!==3)throw new Error("Schema de backup incompatível. Use um backup completo do Firestore V22.8.5 a V22.9.25 com schema 3.");
  if(!["22.8.5","22.8.6","22.8.7","22.8.8","22.8.9","22.9.0","22.9.1","22.9.2","22.9.3","22.9.4","22.9.5","22.9.6","22.9.7","22.9.8","22.9.9","22.9.10","22.9.11","22.9.12","22.9.13","22.9.14","22.9.15","22.9.16","22.9.17","22.9.18","22.9.19","22.9.20","22.9.21","22.9.22","22.9.23","22.9.24","22.9.25"].includes(String(payload.version||"")))throw new Error("Versão de backup incompatível.");
  ...
  if(!Array.isArray(payload.collections.payments)&&!["22.9.23","22.9.24","22.9.25"].includes(String(payload.version||"")))payload.collections.payments=[];
  ...
  if(["22.9.23","22.9.24","22.9.25"].includes(String(payload.version||""))&&payload.collections.payments.some(item=>!Number.isSafeInteger(item.quantity)||item.quantity<1||item.quantity>99000000))throw new Error("Pagamento sem quantidade válida no backup.");
  ...
  for(const claim of payload.collections.nicknameClaims){
    ...
    if(["22.9.12","22.9.13","22.9.14","22.9.15","22.9.16","22.9.17","22.9.18","22.9.19","22.9.20","22.9.21","22.9.22","22.9.23","22.9.24","22.9.25"].includes(String(payload.version||""))&&String(user.memberDocumentId||"")!==String(claim.memberId||""))throw new Error(`Documento de membro divergente na reserva ${claim.id}.`);
    if(["22.9.12","22.9.13","22.9.14","22.9.15","22.9.16","22.9.17","22.9.18","22.9.19","22.9.20","22.9.21","22.9.22","22.9.23","22.9.24","22.9.25"].includes(String(payload.version||""))&&normalizeAccessRole(user.accessRole||user.role)!=="dev"&&!claim.memberId)throw new Error(`Reserva sem membro vinculado: ${claim.id}.`);
  ...
  if(["22.9.11","22.9.12","22.9.13","22.9.14","22.9.15","22.9.16","22.9.17","22.9.18","22.9.19","22.9.20","22.9.21","22.9.22","22.9.23","22.9.24","22.9.25"].includes(String(payload.version||"")))for(const user of payload.collections.users){
  ...
  for(const [backupId,data] of Object.entries(groups)){
    ...
    if(["22.9.17","22.9.18","22.9.19","22.9.20","22.9.21","22.9.22","22.9.23","22.9.24","22.9.25"].includes(String(payload.version))&&parent.status!=="completed")throw new Error(`Backup semanal incompleto: ${backupId}.`);
