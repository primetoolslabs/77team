...
function owner(){return state.profile?.role==="dev"||state.profile?.role==="owner"}
function leadership(){return state.profile?.role==="leadership"||state.profile?.role==="lideranca"}
function staff(){return state.profile?.role==="staff"}
...
on("checkUpdatesButton","click",()=>{setText("updateStatusText","Versão 16.0 instalada. Integração automática aguardando backend seguro.");toast("Verificação local concluída.")});
on("createBackupButton","click",()=>{if(!owner())return;downloadJson(`77-team-backup-${new Date().toISOString().slice(0,10)}.json`,{version:"16.0",exportedAt:new Date().toISOString(),members:state.members,attendance:state.attendance,users:state.users,events:state.events,notifications:state.sentNotifications,audit:state.audit,settings:state.settings});toast("Backup JSON gerado.")});
...
document.addEventListener("click",async e=>{
  ...
  if(toggleStaff){
    if(!owner())return toast("Somente o DEV pode alterar cargos administrativos.");
    ...
    if(user.role==="owner"){
      return toast("O cargo DEV não pode ser alterado.");
    }
  ...
  if(approve&&editor()){
    ...
    if(accessRole==="staff"&&!owner()){
      return toast("Somente o DEV pode promover um usuário para Staff.");
    }
