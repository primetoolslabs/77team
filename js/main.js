...
function canOpenPage(page){
  if(RESTRICTED_EDITOR_PAGES.has(page))return editor();
...
function render(){
  ...
  renderNotifications();renderCalendar();renderStatistics();updatePdfMemberOptions();renderOwnProfile();applyRestrictedVisibility();
}
...
on("profileNicknameForm","submit",async event=>{
  ...
  try{
    ...
    state.profile.name=name;applyPermissions();renderOwnProfile();applyRestrictedVisibility();toast("Nickname atualizado.");
...
on("profileAvatarForm","submit",async event=>{
  ...
  try{
    ...
    renderOwnProfile();applyRestrictedVisibility();
    applyPermissions();
    event.target.reset();
...
on("removeProfileAvatar","click",async()=>{
  try{
    ...
    state.profile.avatarDataUrl="";renderOwnProfile();applyRestrictedVisibility();toast("Avatar removido.");
