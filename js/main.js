...
function render(){
  ...
  renderNotifications();renderCalendar();renderStatistics();updatePdfMemberOptions();renderOwnProfile();
}
...
on("profileNicknameForm","submit",async event=>{
  ...
  try{
    ...
    state.profile.name=name;applyPermissions();renderOwnProfile();toast("Nickname atualizado.");
  }catch(error){toast(error.message||"Não foi possível atualizar.")}
});
on("profileAvatarForm","submit",async event=>{
  event.preventDefault();
  try{
    const avatarDataUrl=await resizeAvatar(byId("profileAvatarInput")?.files?.[0]);
    await updateDoc(doc(db,"users",state.user.uid),{avatarDataUrl,updatedAt:serverTimestamp()});
    state.profile.avatarDataUrl=avatarDataUrl;renderOwnProfile();event.target.reset();toast("Avatar atualizado.");
  }catch(error){toast(error.message||"Não foi possível atualizar o avatar.")}
});
on("removeProfileAvatar","click",async()=>{
  try{
    await updateDoc(doc(db,"users",state.user.uid),{avatarDataUrl:"",updatedAt:serverTimestamp()});
    state.profile.avatarDataUrl="";renderOwnProfile();toast("Avatar removido.");
