...
$("#bootstrapForm").addEventListener("submit",async event=>{
  ...
  try{
    ...
    if(email!==OWNER_EMAIL.toLowerCase())throw new Error("E-mail não autorizado.");
    ...
    await setDoc(doc(db,"users",credential.user.uid),{
      name:"Pimentel",email,role:"owner",active:true,status:"approved",
      createdAt:serverTimestamp()
    });
...
async function recoverOwnerProfile(firebaseUser){
  ...
  if(email!==OWNER_EMAIL.toLowerCase())return null;
  ...
  const recovered={name:"Pimentel",email:OWNER_EMAIL,role:"owner",active:true,status:"approved",recoveredAt:serverTimestamp()};
