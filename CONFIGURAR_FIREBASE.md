...
- Proprietário: definido no primeiro acesso
...
3. Informe o e-mail que será o proprietário definitivo.
...
## Proprietário flexível

O e-mail do proprietário não fica fixo no código.

O primeiro usuário que concluir **Primeiro acesso** cria:

- conta Authentication;
- perfil `users/UID` com `role: owner`;
- documento único `system/owner`.

Depois que `system/owner` existir, o primeiro acesso fica bloqueado pelas regras.

### Atenção

Publique o sistema e faça o primeiro acesso imediatamente.
Enquanto `system/owner` ainda não existir, qualquer pessoa com o link poderia tentar
ser o primeiro proprietário.
