# 77 TEAM Manager V22.9.2 Hotfix — ADMINISTRAÇÃO

Esta versão conclui a revisão funcional da categoria **ADMINISTRAÇÃO** e mantém as melhorias do AVANÇADO e de Cargos e Permissões. O Firebase App Check continua removido; mantenha Firestore e Storage em **Monitorando** ou **Não aplicado**. Consulte `CHANGELOG-V22.9.2.md` e `PUBLICACAO-V22.9.2.md`.
...
## V22.5.3 Hotfix — Correção completa de permissões

- Firestore agora usa `accessRole` como fonte preferencial de permissão.
- Perfis antigos continuam compatíveis com `systemRole`, `permissionRole`, `userRole`, `cargo` e `role`.
- Aprovação por Liderança e Staff passa a aceitar a gravação controlada de `accessRole`.
- Alteração de cargos em contas aprovadas foi sincronizada entre `role`, `accessRole` e `memberRole`.
- Liderança não pode promover ou alterar DEV/Liderança.
- Staff só pode alterar cargos de membros e não pode conceder acesso administrativo.
- Usuários não podem modificar seus próprios campos de permissão.
- Segmentação de notificações usa o cargo efetivo, evitando divergência entre interface e Firestore.
- Login, Firebase, módulos e interface da V22.5.2 foram preservados.

## V22.6.1 Hotfix — Login e Permissões

- Corrigida a leitura do próprio perfil durante o login.
- Adicionada recuperação segura do perfil do DEV quando `system/owner` existe.
- Contas antigas sem `status` passam a ser tratadas como aprovadas, desde que não estejam desativadas.
- Melhoradas as mensagens de erro de autenticação e conexão.
- Adicionada persistência de sessão através de “Lembrar meu acesso”.
- Mantida a matriz de aprovação: DEV, Liderança e Staff conforme seus limites.


## V22.6.1 — Exclusão de personagem

- Adicionado o botão **Excluir** ao lado de Ver detalhes, Gerar PDF e Editar.
- A exclusão remove somente os dados do personagem; conta, UID, cargo, histórico e e-mail permanecem ativos.
...
## V22.6.6 Stable — Personalização do Login
